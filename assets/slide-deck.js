(function(){
  ...
  function splitText(text){const parts=text.match(/[^.!?;:]+[.!?;:]?/g)||[text]; const chunks=[]; for(let i=0;i<parts.length;i+=2){chunks.push(parts.slice(i,i+2).join(' ').trim())} return chunks.filter(Boolean)}
  function splitNode(node){
    if(!node) return [];
    if(node.matches && node.matches('table,.formula,.definition')) return [wrap(node.outerHTML)];
    if(node.matches && node.matches('ul,ol')){const items=[...node.children]; const out=[]; for(let i=0;i<items.length;i+=3){const list=node.cloneNode(false); items.slice(i,i+3).forEach(li=>list.appendChild(li.cloneNode(true))); out.push(wrap(list.outerHTML));} return out;}
